      ...
      try {
        ...
        if (tableName === "products" && tableData.length > 0) {
          ...
          if (backupProductIds.length > 0) {
            ...
            for (let i = 0; i < backupProductIds.length; i += deleteBatchSize) {
              ...
              if (deleteError) {
                console.error(
                  `Error deleting specific products from ${tableName}:`,
                  deleteError,
                );
                deleteErrors++;
              } else {
          ...
          while (hasMore) {
            ...
            if (fetchError) {
              console.error(
                `Error fetching data to delete from ${tableName}:`,
                fetchError,
              );
              deleteErrors++;
              break;
            ...
            for (let i = 0; i < existingData.length; i += deleteBatchSize) {
              ...
              if (ids.length > 0) {
                ...
                if (deleteError) {
                  console.error(
                    `Error deleting batch from ${tableName}:`,
                    deleteError,
                  );
                  deleteErrors++;
                } else {
        ...
        if (tableName === "products" && tableData.length > 0) {
          ...
          if (categoryIds.length > 0) {
            ...
            const invalidCategoryProducts = tableData.filter(
              (row: any) =>
            ...
            if (invalidCategoryProducts.length > 0) {
              console.warn(
                `Found ${invalidCategoryProducts.length} products with invalid category_id. They will be set to null.`,
              );
              // Set invalid category_id to null
              tableData.forEach((row: any) => {
                if (
                  row.category_id &&
                  !existingCategoryIds.has(row.category_id)
                ) {
        ...
        for (let i = 0; i < tableData.length; i += batchSize) {
          ...
          const cleanBatch = batch
            .map((row: any) => {
              ...
              if (tableName === "products") {
                ...
                if (
                  cleanRow.status &&
                  !["draft", "active", "archived"].includes(cleanRow.status)
                ) {
                ...
                if (
                  cleanRow.inventory_policy &&
                  !["continue", "deny"].includes(cleanRow.inventory_policy)
                ) {
                ...
                if (cleanRow.track_inventory === undefined)
                  cleanRow.track_inventory = true;
                if (cleanRow.inventory_quantity === undefined)
                  cleanRow.inventory_quantity = 0;
                if (cleanRow.low_stock_threshold === undefined)
                  cleanRow.low_stock_threshold = 5;
                if (cleanRow.is_featured === undefined)
                  cleanRow.is_featured = false;
                if (cleanRow.is_digital === undefined)
                  cleanRow.is_digital = false;
                if (cleanRow.requires_shipping === undefined)
                  cleanRow.requires_shipping = true;
              }
          ...
          if (cleanBatch.length > 0) {
            ...
            if (insertError) {
              ...
              errorDetails.push(
                insertError.message || insertError.code || "Unknown error",
              );
        ...
        restoreResults[tableName] = {
          ...
          problematic_records:
            problematicRecords.length > 0
              ...
              : undefined, // Limit to first 3 for UI
          // Add note if deletion had issues but restoration was successful
          note:
            deleteErrors > 0 && allRecordsRestored && !hasInsertErrors

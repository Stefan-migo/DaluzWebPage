...
function CheckoutSuccessContent() {
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        const response = await fetch(
          `/api/checkout?order_id=${external_reference}`,
        );
        const data = await response.json();

        if (!response.ok) {
          throw new Error(
            data.error || "Error al obtener detalles de la orden",
          );
        }
  ...
  // Track purchase event to GA4 and FB Pixel when payment is approved
  ...
  usePurchaseTracking(
    isApproved ? orderDetails?.order_id || "" : "",
    isApproved ? orderDetails?.total_amount || 0 : 0,
    isApproved ? orderItems : [],
  );

  const getStatusInfo = () => {
    const statusLower =
      status?.toLowerCase() || collection_status?.toLowerCase();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="mb-8 border-2 border-verde-suave/20">
          <CardContent className="p-8 text-center">
            ...
            <Badge className={statusInfo.color}>{statusInfo.badge}</Badge>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-dorado flex items-center justify-center text-white text-sm font-bold">
                  1
                </div>
                <div>
                  <p className="font-semibold">Confirmación por Email</p>
                  <p className="text-sm text-gris-medio">
                    Te enviaremos un email con todos los detalles de tu orden
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gris-medio">
                    Preparamos cuidadosamente tus productos naturales
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">

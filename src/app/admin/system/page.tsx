...
export default function SystemAdministrationPage() {
  ...
  const fetchSystemData = async () => {
    try {
      setLoading(true);
      await Promise.all([fetchConfigs(), fetchHealthMetrics()]);
      setError(null);
    } catch (err) {
  ...
  const confirmRestoreBackup = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Error al restaurar backup",
      );
    } finally {
  ...
  const handleViewBackupDetails = async (backup: any) => {
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Error al cargar detalles del backup",
      );
    }
  };
  ...
  const confirmDeleteBackup = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/system/backups?filename=${encodeURIComponent(selectedBackup.filename)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleClearMemory = async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Error al limpiar memoria",
      );
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-admin-error-text mb-2">
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex space-x-2">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 ${refreshingHealth ? "animate-spin" : ""}`}
            />
            Actualizar Estado
          </Button>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-[#AE0000]">
            ...
            <Card className="bg-admin-bg-secondary admin-card">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    className="justify-start btn-daluz-outline"
            ...
            <Card className="bg-admin-bg-secondary admin-card">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  >
                    <RefreshCw
                      className={`h-3 w-3 ${refreshingHealth ? "animate-spin" : ""}`}
                    />
                  </Button>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {healthMetrics.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    >
                      <RefreshCw
                        className={`h-3 w-3 mr-2 ${refreshingHealth ? "animate-spin" : ""}`}
                      />
                      Recolectar Métricas
                    </Button>
                  </div>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                <div className="flex-1">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(configs.map((c) => c.category))).map(
                        (category) => {
                ...
                <div>
                  ...
                  <Button className="w-full md:w-auto btn-daluz-outline">
                    {showSensitive ? (
                      <>
          ...
          {Object.keys(configsByCategory).length === 0 ? (
            <Card className="bg-admin-bg-secondary admin-card">
              <CardContent className="p-12 text-center">
                ...
                />
                <p style={{ color: "var(--admin-text-secondary)" }}>
                  No se encontraron configuraciones con los filtros
                  seleccionados
                </p>
              </CardContent>
            </Card>
          ) : (
            Object.entries(configsByCategory).map(
              ([category, categoryConfigs]) => {
                ...
                return (
                  <Card
                    key={category}
                    className="bg-admin-bg-secondary admin-card"
                  >
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        <div className="flex items-center">
                        ...
                        <Badge variant="default">
                          {(categoryConfigs as SystemConfig[]).length}{" "}
                          {(categoryConfigs as SystemConfig[]).length === 1
                            ? "configuración"
                            : "configuraciones"}
                        </Badge>
                      </CardTitle>
                      ...
                      {healthMetrics.map((metric) => (
                        <TableRow key={metric.id}>
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                metric.is_healthy ? "healty" : "destructive"
                              }
                            >
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-1">
                                <span className="font-semibold text-admin-error-text dark:text-red-400">
                                  Crítico
                                </span>
                                <Badge
                                  variant="destructive"
                              ...
                              <p className="text-sm text-admin-error-text dark:text-red-400 mb-2">
                                Valor actual:{" "}
                              ...
                              <p className="text-xs text-[#AE0000] dark:text-red-500 bg-red-100 dark:bg-red-900/30 p-2 rounded mt-2">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-1">
                                ...
                                {metric.metric_name === "memory_usage" && (
                                  <Button
                                    className="h-7 text-xs btn-daluz-outline"
                                    size="sm"
                                    onClick={handleClearMemory}
                                    disabled={clearingMemory}
                                  >
          ...
          <Card className="bg-admin-bg-secondary admin-card">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Button
                  className="justify-start h-auto p-4 flex-col items-start btn-daluz-outline"
                ...
                >
                  ...
                  {maintenanceAction === "backup_database" &&
                    maintenanceLoading && (
                ...
                >
                  ...
                  {maintenanceAction === "optimize_database" &&
                    maintenanceLoading && (
                      <RefreshCw className="h-3 w-3 animate-spin mt-2" />
                    )}
                </Button>
                ...
                >
                  ...
                  {maintenanceAction === "security_audit" &&
                    maintenanceLoading && (
                      <RefreshCw className="h-3 w-3 animate-spin mt-2" />
                    )}
                </Button>
                ...
                >
                  ...
                  {maintenanceAction === "system_status" &&
                    maintenanceLoading && (
                      <RefreshCw className="h-3 w-3 animate-spin mt-2" />
                    )}
                </Button>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button
                            className="text-xs btn-daluz-outline"
                            size="sm"
                          ...
                          >
                            <RotateCcw
                              className={`h-3 w-3 mr-1 ${restoringBackup ? "animate-spin" : ""}`}
                            />
                            Restaurar
                          </Button>
                          <Button
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {restoreResults.errors > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-[#AE0000]">
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                {result.status === "error" && (
                                  <XCircle className="h-4 w-4 text-[#AE0000]" />
                                )}
                              ...
                              >
                                ...
                                {result.status === "error" && (
                                  <span className="text-admin-error-text dark:text-red-400">
                                    Error
                                  </span>
                                )}
                              </div>
                            ...
                            {result.error && (
                              <p className="text-xs text-[#AE0000] dark:text-red-400 mt-1 ml-6">
                                {result.error}
                              </p>
                            )}
                            {result.error_message && (
                              <p className="text-xs text-[#AE0000] dark:text-red-400 mt-1 ml-6 font-semibold">
                                Error: {result.error_message}
                              </p>
                            )}
                            {result.problematic_records &&
                              result.problematic_records.length > 0 && (
                                <div className="mt-2 ml-6 space-y-1">
                                  <p className="text-xs font-semibold text-admin-error-text dark:text-red-400">
                                    Registros problemáticos (
                                    {result.problematic_records.length}):
                                  </p>
                                  {result.problematic_records.map(
                                    (pr: any, idx: number) => (
                                      <div
                                        key={idx}
                                        className="text-xs text-[#AE0000] dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-2 rounded border border-red-200 dark:border-red-800"
                                      >
          ...
          {selectedBackup && (
            <div className="space-y-4">
              <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="h-5 w-5 text-[#AE0000] flex-shrink-0 mt-0.5" />
                  <div className="flex-1">
                    <p className="font-semibold text-red-800 dark:text-red-300 mb-2">
                      ⚠️ Advertencia
                    </p>
                    <p className="text-sm text-admin-error-text dark:text-red-400">
